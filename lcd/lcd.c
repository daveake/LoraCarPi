...
struct TLabelledBox
{
	int x, y, width, height, labelfontsize, valuefontsize;
	char *label, *format;
};

struct TLabelledBox BalloonPayloadBox =		{442, 390, 270, 50, 18, 32, "Payload Name", "%s"};
struct TLabelledBox BalloonAltitudeBox =	{442, 296, 270, 50, 18, 32, "HAB Altitude", "%5.0lfm"};
struct TLabelledBox BalloonRateBox =	 	{442, 202, 270, 50, 18, 32, "Ascent Rate", "%5.1lfm/s"};
struct TLabelledBox DistanceToHABBox =	 	{442, 108, 270, 50, 18, 32, "Distance To HAB", "%5.1lfm"};
struct TLabelledBox PacketRSSIBox =	 		{442,  14, 270, 50, 18, 32, "Packet RSSI", "%3.0lfdBm"};
	
struct TStatusCircle
{
	int x, y, width, height, labelfontsize;
	char *label;
};

struct TStatusCircle ChaseStatus =	{40, 44, 40, 44, 24, "C"};
struct TStatusCircle batcStatus =	{380, 44, 40, 44, 24, "B"};
struct TStatusCircle GPSStatus =	{380, 435, 40, 44, 24, "G"};
struct TStatusCircle HABStatus =	{40, 435, 40, 44, 24, "H"};
...
void DrawLabelledBox(struct TLabelledBox Box)
{
	VGfloat dotcolor[4] = {0, 0, 0, 0.3};
	...
	RGB(0,0,0,dotcolor);
	setfill(dotcolor);
	Text(Box.x, Box.y+Box.height+8, Box.label, SansTypeface, Box.labelfontsize);
	
	// Box
	Stroke(64, 64, 64, 0.5);
	StrokeWidth(2);
	RGB(128, 128, 128, dotcolor);
	setfill(dotcolor);
	Rect(Box.x, Box.y, Box.width, Box.height);
}

void ShowLabelledBoxString(struct TLabelledBox Box, char *Value)
{
	VGfloat dotcolor[4] = {0, 0, 0, 0.3};
	...
	RGB(0,0,0,dotcolor);
	setfill(dotcolor);
	TextEnd(Box.x+Box.width-16, Box.y+8, Value, SansTypeface, Box.valuefontsize);
}
...
void ShowRSSI(int RSSI)
{
	...
	for (Bar=1; Bar<=10; Bar++)
	{
		if (Bar <= Bars)
		{
			RGB(16, 16, 48, dotcolor);
		}
		else
		{
			RGB(80, 80, 80, dotcolor);
		}
		...
		// RGB(128, 128, 128, dotcolor);
		setfill(dotcolor);
		Rect(64 + 25 * Bar, 110, 20, 40);
	}
}

void ShowLabelledBoxValue(struct TLabelledBox Box, double Value)
{
	char temp[16];
	
	sprintf(temp, Box.format, Value);
	ShowLabelledBoxString(Box, temp);
}

void ShowCompassDirection(float Angle)
{
	float x, y;
	...
	FillRadialGradient(240, 240, 20, 20, 440, NULL, 3);
	Ellipse(210, 240, 410, 464);
	// VGfloat dotcolor[4] = {0, 0, 0, 0.3};

	// RGB(0, 0, 128, dotcolor);
	// setfill(dotcolor);
	Stroke(0, 0, 192, 0.5);
	StrokeWidth(8);
	
	Line(210, 240, 210+x, 240+y);
}


void Layout(int w, int h) {
	// char *para[] = {
	//	"UKHAS Chase Car Pi Application",
	//	NULL
	// };
	// char line[80];
	
	// VGfloat lmargin = w * 0.60, top = h * .7, mid = h * .5, bot = h * .3;

	// int fontsize = 24, leading = 40;

	Start(w, h);

	// Background
	FillLinearGradient(w-1, h-1, 0, 0, NULL, 3);
	Rect(0, 0, w, h);
	
	// Boxes
	DrawLabelledBox(BalloonPayloadBox);
	DrawLabelledBox(BalloonAltitudeBox);
	DrawLabelledBox(BalloonRateBox);
	DrawLabelledBox(DistanceToHABBox);
	DrawLabelledBox(PacketRSSIBox);

	End();
}

void ShowStatusCircle(struct TStatusCircle Status, int Active)
{
	VGfloat dotcolor[4] = {0, 0, 0, 0.3};
	...
	if (Active)
	{
		RGB(0, 128, 0, dotcolor);
	}
	else
	{
		RGB(128, 0, 0, dotcolor);
	}
	setfill(dotcolor);
	Ellipse(Status.x, Status.y, Status.width, Status.height);
	
	RGB(0,0,0,dotcolor);
	setfill(dotcolor);
	Text(Status.x - 12, Status.y - 11, Status.label, SansTypeface, Status.labelfontsize);
}

float CalculateDistance(float HABLatitude, float HABLongitude, float CarLatitude, float CarLongitude)
{
    ...
    return 6371000 * acos(sin(CarLatitude) * sin(HABLatitude) +
                     cos(CarLatitude) * cos(HABLatitude) * cos(HABLongitude-CarLongitude));
}

float CalculateDirection(float HABLatitude, float HABLongitude, float CarLatitude, float CarLongitude)
{
    float x, y;
...
int main(int argc, char **argv)
{
    ...
    int   id;
    int keypress;
	float DistanceToHAB, DirectionToHAB;

	printf("Raspberry Pi LCD Program for Chase Car\r\n\n");
	...
	{
        keypress = getchar();
        if (keypress == 0x1b || keypress == '\n') {
            break;
        }
		
        DistanceToHAB = CalculateDistance(HAB->HAB_Latitude, HAB->HAB_Longitude, HAB->GPS_Latitude, HAB->GPS_Longitude);
        DirectionToHAB = CalculateDirection(HAB->HAB_Latitude, HAB->HAB_Longitude, HAB->GPS_Latitude, HAB->GPS_Longitude) - HAB->GPS_Direction;

		ShowLabelledBoxString(BalloonPayloadBox, HAB->HAB_Payload);
		ShowLabelledBoxValue(BalloonAltitudeBox, HAB->HAB_Altitude);
		ShowLabelledBoxValue(BalloonRateBox, HAB->HAB_AscentRate);
		ShowLabelledBoxValue(DistanceToHABBox, DistanceToHAB);
		ShowLabelledBoxValue(PacketRSSIBox, HAB->PacketRSSI);

		ShowStatusCircle(ChaseStatus, HAB->car_status);
		ShowStatusCircle(batcStatus, HAB->batc_status);
		ShowStatusCircle(HABStatus, HAB->HAB_status);
		ShowStatusCircle(GPSStatus, HAB->GPS_Satellites >= 4);
		
		ShowCompassDirection(DirectionToHAB);
		
		ShowRSSI(HAB->CurrentRSSI);
